...
2. **文本类型**：
   ```
   用户输入: Hello World
   二维码内容: https://negiao-pages.share.connect.posit.cloud/?content=Hello%20World&type=文本&...
   ```

3. **联系方式**：
   ```
   用户输入: 姓名、电话、邮箱等
   二维码内容: https://negiao-pages.share.connect.posit.cloud/?vcard={"name":"张三",...}&type=名片&...
   ```
